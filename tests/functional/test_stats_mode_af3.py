...
@pytest.fixture
def temp_dir(
    test_case: StatsModeTestCase, tmp_path_factory: pytest.TempPathFactory
) -> Path:
    """Create a unique temporary directory for each test case."""
    return tmp_path_factory.mktemp(
        f"data_{test_case.ligand_name}_{os.urandom(8).hex()}"
    )
...
def cli_output(temp_dir: Path, test_case: StatsModeTestCase) -> List:
    ...
    ), f"Status file does not contain success status for {ligand_name}"
...
def validated_ligands(validated_structure: List) -> List:
    ...
    for entry in validated_structure:
        for ligand in entry["ligands"]:
            ...
            assert isinstance(
                ligand["procrustes"], float
            ), "Procrustes should be a float"
            assert isinstance(
                ligand["coordination"], int
            ), "Coordination should be an integer"
            assert isinstance(ligand["count"], int), "Count should be an integer"
            assert isinstance(
                ligand["description"], str
            ), "Description should be a string"

    return validated_structure


def test_main_func_stats_af3_cif_with_validation(
    validated_ligands: List, test_case: StatsModeTestCase
):
    ...
    for entry in al_entries:
        for ligand in entry["ligands"]:
            assert (
                ligand["coordination"] == 6
            ), f"Aluminum should have coordination number 6 for {test_case.ligand_name}"
...
def test_specific_af3_properties(validated_ligands: List, test_case: StatsModeTestCase):
    ...
    for entry in validated_ligands:
        assert (
            entry["residue"] == "AF3"
        ), f"Expected AF3 residue for {test_case.ligand_name}"

        # Test for expected ligand properties
        for ligand in entry["ligands"]:
            if ligand["class"] == "octahedral":
                assert (
                    ligand["procrustes"] < 0.1
